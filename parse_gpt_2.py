...
def print_gpt_partition_entry_data(list_gpt_partition_entry):
    ...
    for i in range(len(list_gpt_partition_entry)):
        ...
        print(f'   [-] Partition Type GUID: {(list_gpt_partition_entry[i]["Partition Type GUID"])}')
...
def parse_gpt_partition_table(disk_path, gpt_header_struct):
    ...
    while(gpt_cnt_entry < 128):
        gpt_partition_entry_struct = {}
        gpt_entry_offset = gpt_cnt_entry * gpt_header_struct["entry_size"]

        def parse_Partition_Type_GUID(gpt_partition_entry_data, gpt_entry_offset):    

            part1 = str(hex(struct.unpack("<I", gpt_partition_entry_data[gpt_entry_offset:gpt_entry_offset+0x4])[0])[2:])
            part2 = str(hex(struct.unpack("<H", gpt_partition_entry_data[gpt_entry_offset+0x4:gpt_entry_offset+0x6])[0])[2:])
            part3 = str(hex(struct.unpack("<H", gpt_partition_entry_data[gpt_entry_offset+0x6:gpt_entry_offset+0x8])[0])[2:])
            part4 = str(hex(struct.unpack(">Q", gpt_partition_entry_data[gpt_entry_offset+0x8:gpt_entry_offset+0x10])[0])[2:])

            combined_part = (part1 + "-" + part2 + "-" + part3 + "-"+ part4[:4] + "-" + part4[4:])
            return combined_part
        
        gpt_partition_entry_struct["Partition Type GUID"] = parse_Partition_Type_GUID(gpt_partition_entry_data, gpt_entry_offset)
        gpt_partition_entry_struct["Unique Partition GUID"] = gpt_partition_entry_data[gpt_entry_offset+0x10:gpt_entry_offset+0x20]
